# contract-history Specification

## Purpose
What a party asked to act on a contract is told about that contract's
history: the amendment records rendered beside it, so a criterion added after a
round of review is not indistinguishable from one written when the task opened.
Built from records, never by reading the contract document for history.
