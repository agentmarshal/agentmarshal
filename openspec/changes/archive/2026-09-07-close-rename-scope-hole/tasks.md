...
- [x] 1.1 `run_gate` derives `changed` from the decomposed listing and `_changed_paths` is deleted — verify: `grep -n "name-only" src/agentmarshal/journal/gate.py` finds no `diff` listing; what remains is `ls-tree` and `log` output.
