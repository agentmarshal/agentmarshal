...
directory beside the journal, rather than anywhere a tool would treat as
separate from it. Any wrapper that commits a journal transaction the obvious
way — stage the journal directory, commit, open a pull request — sweeps
whatever findings happen
